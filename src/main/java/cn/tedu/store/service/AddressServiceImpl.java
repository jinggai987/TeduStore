...
public class AddressServiceImpl implements AddressService {
	...
	public void addAddress(Address address) {
		...
		// 判断当前用户的收货人数据的数量是否为0
		Integer count 
			= addressMapper.getRecordCountByUid(address.getUid());
		if (count == 0) {
	...
	public void delete(Integer id, Integer uid) {
		...
		if (addr.getIsDefault() == 1) {
			...
			Integer newDefaultAddressId
				= addressMapper.getFirstRecordId(uid);
			// 设置新的默认收货人
	...
	private void generateAddressDistrict(Address address) {
		String provinceName 
			= dictService.getProvinceNameByCode(address.getRecvProvince());
		String cityName 
			= dictService.getCityNameByCode(address.getRecvCity());
		String areaName 
			= dictService.getAreaNameByCode(address.getRecvArea());
